...
def async_audio():
    ...
    for i,table in enumerate(sorted(listdir(path))):
        phrases,words,langs,deck_name=data_from_starting_table('v',path+'//'+table)
        create_audios(cardtype,langs,phrases,'tests//audio_tests',f'exemplo{i}',words)
...
def test_async_translate():
    ...
    for i,table in enumerate(sorted(listdir(path))):
        phrases,words,langs,deck_name=data_from_starting_table('v',path+'//'+table)
        trans_phrases,trans_words=translate_phrases_n_words(cardtype,langs,chosen_lang,phrases,words)
